...
- (void)BaseUse {
    ...
    if (UUserDefaults.isLogin) {
        ...
        //如果已经 置空了，只能创新创建
        [self zm_addNavItemRect:CGRectMake(0,0,40, 20) title:@"登录" imgName:nil tag:222 isRightItem:YES];
    }
    ...
    // 返回按钮：其他情况返回
    [self zm_backBtnNoNavBar:NO normalBack:NO];
    self.backBlock = ^(id object) {
        NSLog(@"--> backBlock");
    };
    
    //暂无数据，点击刷新
    [self zm_addNoDataBtnToSuperview:self.view];
    self.noDataRefreshBlock = ^{
        //NSLog(@"---> noDataRefreshBlock 马上进入刷新状态");
    };
    
    //是否隐藏导航栏最下的一条线
    [self zm_navBarlineImgViewHidden:YES];
    [self zm_navBarlineImgViewHidden:NO];
    [self zm_navBarlineImgViewChangeFrame];
}
...
// 普通：提示信息
- (void)zm_hudShowMessage:(NSString *)message afterDelay:(NSTimeInterval)delay {
    [HHudProgress hudShowMsg:message delay:delay addSubview:self.view];
}
// 成功时：提示信息
- (void)zm_hudShowSucceed:(NSString *)message afterDelay:(NSTimeInterval)delay {
    [HHudProgress hudShowMsg:message delay:delay addSubview:self.view];
}
