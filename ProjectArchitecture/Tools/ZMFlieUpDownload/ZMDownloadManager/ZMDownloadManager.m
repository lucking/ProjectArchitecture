 ...
 *  开启任务下载资源
 */
- (void)zm_downloadURL:(NSString *)url progress:(void (^)(NSInteger, NSInteger, CGFloat))progressBlock state:(void (^)(DownloadState))stateBlock
{
    if (!url) return;
...
- (void)test_use {
    ...
    NSString *urlStr = @"http://cartoon.zhuishushenqi.com/api/v1/bookcenter/scenethumbnail/1/2066653/U_4_2_cf22965b-5b83-4a56-8f96-8e3f1aab099e.png/2/2";
    urlStr = @"http://img.zcool.cn/community/01f17956138a5832f875a132eb1d85.jpg@1280w_1l_2o_100sh.jpg";
    // 视频下载
    [[ZMDownloadManager sharedInstance] zm_downloadURL:urlStr progress:^(NSInteger receivedSize, NSInteger expectedSize, CGFloat progress) {
        
        dispatch_async(dispatch_get_main_queue(), ^{
            NSLog(@"---progress = %f",progress);
            if (progress ==1.0) {
                NSLog(@"---> getFile = %@",[[ZMDownloadManager sharedInstance] getFile:urlStr]);
            }
        });
