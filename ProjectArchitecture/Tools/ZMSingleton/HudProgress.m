...
- (void)hudShowLoadingAddSubview:(UIView *)superView
{
    ...
    _hudView = [self hudAddSubview:superView];
}
// 菊花 延时
                   ...
                   delay:(NSTimeInterval)delay addSubview:(UIView *)superView
{
    ...
    MBProgressHUD *hud = [self hudAddSubview:superView];
    hud.mode = MBProgressHUDModeText;
    hud.detailsLabel.text = NSLocalizedString(message, @"HUD message detail title");
    hud.offset = CGPointMake(0, 0);
    hud.margin = margin;
    [hud hideAnimated:YES afterDelay:delay];
    _hudView = hud;
             ...
             delay:(NSTimeInterval)delay addSubview:(UIView *)superView
{
    ...
    MBProgressHUD *hud = [self hudAddSubview:superView];
    hud.mode = MBProgressHUDModeText;
    if (message) {   hud.label.text        = NSLocalizedString(message, @"HUD message title");}
    if (detailMsg) { hud.detailsLabel.text = NSLocalizedString(detailMsg, @"HUD message detail title");}
    if (msgFont) {   hud.label.font        = [UIFont systemFontOfSize:msgFont];}
    if (detailFont) {hud.detailsLabel.font = [UIFont systemFontOfSize:detailFont];}    
    hud.offset = CGPointMake(0, 0);
    hud.margin = margin;
    [hud hideAnimated:YES afterDelay:delay];
    _hudView = hud;
